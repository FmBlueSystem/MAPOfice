"""Backward Compatibility Shim for Claude Provider

This module provides backward compatibility for code using the old claude_provider imports.
"""
...
warnings.warn(
    "claude_provider.py is deprecated. Please use the new provider factory: "
...
class ClaudeProvider(BaseLLMProvider):
    """Compatibility wrapper for old ClaudeProvider class"""
    ...
    def _extract_json_from_response(self, text: str):
        ...
        if hasattr(self._provider, '_extract_json_from_response'):
            return self._provider._extract_json_from_response(text)
        import json
        return json.loads(text)
    
    def _create_optimized_prompt(self, track_data):
        """Create optimized prompt"""
        if hasattr(self._provider, '_create_optimized_prompt'):
            return self._provider._create_optimized_prompt(track_data)
